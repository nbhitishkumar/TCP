...
int main(int argc, char *argv[]) {
  ...
  time_t end  = time(NULL) + 7212;
  time_t intervalone,intervaltwo,intervalthree,intervalfour;
  int terminate = 1;
  char buf[mybuf];
  struct sockaddr_in srvaddr;
  struct hostent *host;
  intervalone=time(NULL)+ 1800;
  intervaltwo=intervalone+1800;
  intervalthree=intervaltwo+1800;
  ...
  if (setsockopt(sfd, SOL_TCP, TCP_FASTOPEN, (void *)&flags, sizeof(flags))) 
  { 
...
while(terminate){
      ...
      if (time(NULL)>end) 
      {
          ...
          terminate=0;
            ...
            //fgets(randstring(3),sizeof(randstring),stdin);
            n=write(sfd,randstring(2),strlen(randstring));
            if (n < 0)
            {
	    ...
	    r=r+sizeof(mybuf);
            ...
            //fgets(randstring(3),sizeof(randstring),stdin);
            n=write(sfd,randstring(4),strlen(randstring));
            if (n < 0)
            {
                printf("error wirting  from socket\n");
            }
	    ...
	    r1=r1+sizeof(mybuf);
            ...
            //fgets(randstring(4),sizeof(randstring),stdin);
